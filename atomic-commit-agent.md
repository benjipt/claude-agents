...
description: Use after completing a feature or logical unit of work, BEFORE code-reviewer. This agent commits working code first, ensuring clean history before review. If code-reviewer suggests improvements that get implemented, invoke this agent again to commit those changes (but do NOT run code-reviewer again - one review cycle only). Include your model name (e.g., 'Code generated by Claude Opus 4.5') in the prompt for accurate commit attribution.
